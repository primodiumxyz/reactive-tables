...
export const queries = () => {
  ...
  const passesQueryFragment = <T extends Schema>(record: Record, fragment: RecordQueryFragment<T>): boolean => {
    if (fragment.type === QueryFragmentType.With) {
      ...
      return hasRecord(fragment.table, record);
    }

    if (fragment.type === QueryFragmentType.WithProperties) {
      // Record must have the given table properties
      return recordPropertiesEqual(fragment.properties, getRecordProperties(fragment.table, record));
    }
  ...
  const passesQueryFragmentProxy = <T extends Schema>(
    ...
    for (let i = 0; i < proxyRead.depth; i++) {
      const properties = getRecordProperties(proxyRead.table, proxyEntity);
  ...
  const getChildRecords = (
    ...
    const directChildEntities = getRecordsWithProperties(table, { properties: record });
    if (depth === 1) return directChildEntities;

    const indirectChildEntities = [...directChildEntities]
      .map((childRecord) => [...getChildRecords(childRecord, table, depth - 1)])
      .flat();
      ...
      if (isSettingFragment(fragment)) {
        ...
        records =
          fragment.type === QueryFragmentType.With
            ? new Set([...getTableRecords(fragment.table)])
            : getRecordsWithProperties(fragment.table, fragment.properties);

        // Add record's children up to the specified depth if proxy expand is active
        if (proxyExpand && proxyExpand.depth > 0) {
          for (const record of [...records]) {
            for (const childRecord of getChildRecords(record, proxyExpand.table, proxyExpand.depth)) {
        ...
        for (const record of [...records]) {
          ...
          if (proxyExpand && proxyExpand.depth > 0) {
            const childRecords = getChildRecords(record, proxyExpand.table, proxyExpand.depth);
            for (const childRecord of childRecords) {
              // Add the child record if it passes the direct check
              // or if a proxy read is active and it passes the proxy read check
              if (
                passesQueryFragment(childRecord, fragment) ||
                (proxyRead && proxyRead.depth > 0 && passesQueryFragmentProxy(childRecord, fragment, proxyRead))
              )
                records.add(childRecord);
            }
          }
      ...
      .pipe(
        containsProxy // Query contains proxies
          ? concatMap((update) => {
              ...
              for (const previouslyMatchingRecord of matching) {
                // Record matched before but doesn't match now
                if (!newMatchingSet.has(previouslyMatchingRecord)) {
                  matching.delete(previouslyMatchingRecord);
                  updates.push({
                    record: previouslyMatchingRecord,
                    type: "exit",
                    table: update.table,
              ...
              for (const matchingEntity of newMatchingSet) {
                if (matching.has(matchingEntity)) {
                  ...
                  updates.push({
                    ...
                    table: update.table,
                    properties: {
                      current: getRecordProperties(update.table, matchingEntity),
                      prev: undefined,
                    },
                  });
                } else {
   ...
   * @returns Set of records matching the query fragments.
   */
  const useRecordQuery = (fragments: QueryFragment[], options?: { updateOnValueChange?: boolean }) => {
